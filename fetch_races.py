...
FF_RESERVE    = 50     # keep this many of the 1,000 daily free-tier calls in reserve
FF_CACHE_DAYS = 30     # cache a horse's FormFav profile for 30 days — stats don't change race-to-race
FF_MISS_DAYS  = 7      # remember a "no stats" miss for 7 days before re-checking
FF_TOP_N      = 20     # enrich up to this many runners per race by market (whole field)
FF_MAX_CALLS_PER_RUN = 900   # hard ceiling per run — stays under 1,000/day with reserve
FF_EVERY_HOURS = 24    # one enrichment pass per day — spend the 1k budget carefully
FF_HORIZON_HOURS = 48  # enrich today + tomorrow so the card is ready overnight
FF_PACE_SEC   = 0.15   # no per-second limit confirmed; light pace is polite
...
def _cache_miss(sb, rid, nm):
    """Remember that a runner has no FormFav stats (first-starter / 404) so we don't re-check
    it every run. Stored with stats=None; re-checked only after FF_MISS_DAYS (a first-starter
    may eventually race and get a stats page)."""
    try:
        sb.table("formfav_cache").upsert(
            {"runner_id": rid, "name": nm, "stats": None,
             "fetched_at": datetime.now(timezone.utc).isoformat()},
            on_conflict="runner_id").execute()
    except Exception as e:
        log(f"FormFav miss-cache write failed {rid}: {e}")
...
def enrich_formfav(sb, race_rows, runner_rows, allow_api=True):
    ...
    try:
        ...
        while True:
            chunk = (sb.table("formfav_cache").select("runner_id,name,stats,fetched_at")
                       .range(page * PAGE, page * PAGE + PAGE - 1).execute().data) or []
            rows.extend(chunk)
            if len(chunk) < PAGE:
                break
            page += 1
            if page > 50:   # safety cap (50k rows)
                break
        log(f"FormFav: cache read {len(rows)} rows.")
    except Exception as e:
        log(f"FormFav cache read failed: {e}")
        rows = []
    cutoff = datetime.now(timezone.utc) - timedelta(days=FF_CACHE_DAYS)
    miss_cutoff = datetime.now(timezone.utc) - timedelta(days=FF_MISS_DAYS)
    for r in rows:
        try:
        ...
        if not nm or not fa:
            continue
        has_stats = bool(r.get("stats"))
        # real profile: reuse for FF_CACHE_DAYS. A "no stats" miss: remember only for FF_MISS_DAYS,
        # then let it be re-checked (a first-starter may have raced since).
        if (has_stats and fa > cutoff) or ((not has_stats) and fa > miss_cutoff):
            cache[nm.strip().lower()] = {"id": r["runner_id"], "stats": r["stats"]}
    ...
    for rr in runner_rows:
        if code_by_race.get(rr["race_id"]) == "harness":   # skip harness — enrich thoroughbreds only
            continue
    ...
    if allow_api:
        log(f"FormFav: {len(by_race)} gallops races within {FF_HORIZON_HOURS}h, {len(eligible)} runners eligible (harness skipped).")
    ...
    for rr in order:
        ...
        try:
            ...
            if not results[0].get("lastRaceDate") and (starts in (None, 0)):
                # first-starter / no race history -> FormFav has no stats page (always 404).
                # Cache the miss so we DON'T re-check it every single run.
                seen[key] = {"id": rid, "stats": None}
                _cache_miss(sb, rid, nm)
                continue
            time.sleep(FF_PACE_SEC)
            ...
            if st.status_code == 404:
                # genuinely no stats for this runner -> remember it so we stop re-trying
                seen[key] = {"id": rid, "stats": None}
                _cache_miss(sb, rid, nm)
                continue
            if st.status_code != 200:
                log(f"FormFav stats HTTP {st.status_code} for runnerId {rid} — skipping (transient).")
                seen[key] = {"id": rid, "stats": None}
                continue
            stats = st.json()
            calls += 1
...
def main():
    ...
    def _chunked_upsert(table, rows, conflict, size=200):
        for i in range(0, len(rows), size):
            sb.table(table).upsert(rows[i:i+size], on_conflict=conflict).execute()
    _chunked_upsert("races", race_rows, "id")
    _chunked_upsert("runners", runner_rows, "race_id,selection_id")
