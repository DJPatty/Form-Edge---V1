...
def ff_stats(rid, rt, c, headers):
    ...
    if _ff_stats_builder is not None:
        return requests.get(_ff_stats_builder(rid, rt, c), headers=headers, timeout=20)
    last = None
    for i, b in enumerate(_FF_STATS_CANDIDATES):
        ...
        try:
            ...
            log(f"FormFav stats discovery [{i}] {u.replace(FF_BASE,'')} -> ERROR {e}")
            continue
    ...
    for rr in order:
        ...
        try:
            ...
            if s.status_code == 429:
                log(f"FormFav rate limit reached (HTTP 429) — daily quota exhausted. "
                    f"Resets in ~{_fmt_secs(_ff_reset_secs(s))}. "
                    "No more API calls this run; still applying cached profiles.")
                api_blocked = True
                continue
