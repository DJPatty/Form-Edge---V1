...
FF_TOP_N      = 8      # only enrich the top N runners by market in each race (skip no-hopers)
FF_MAX_CALLS_PER_RUN = 120  # hard ceiling on API calls per run, so one run can't burn the daily quota
...
FF_PACE_SEC   = 1.0         # pause between FormFav calls — avoids the per-second "Rate exceeded" / 500 throttle
FF_BACKOFF_SEC = 8          # if throttled (429/500), wait this long and retry once before giving up
...
# The correct "runner stats by id" path isn't documented to us, so we discover it at runtime:
# try a few likely shapes on the first runner, log each status, lock onto the one that returns 200.
_FF_STATS_CANDIDATES = [
    lambda rid, rt, c: f"{FF_BASE}/stats/runner/{rid}",
    lambda rid, rt, c: f"{FF_BASE}/stats/runner/{rid}?raceType={rt}",
    lambda rid, rt, c: f"{FF_BASE}/stats/runner/{rid}?raceType={rt}&country={c}",
    lambda rid, rt, c: f"{FF_BASE}/stats/runner/{rid}/stats",
    lambda rid, rt, c: f"{FF_BASE}/stats/runner/stats/{rid}",
    lambda rid, rt, c: f"{FF_BASE}/stats/runner?runnerId={rid}",
    lambda rid, rt, c: f"{FF_BASE}/stats/runner?runnerId={rid}&raceType={rt}",
    lambda rid, rt, c: f"{FF_BASE}/runner/{rid}/stats",
    lambda rid, rt, c: f"{FF_BASE}/runner/{rid}",
    lambda rid, rt, c: f"{FF_BASE}/stats/runner/profile/{rid}",
]
_ff_stats_builder = lambda rid, rt, c: f"{FF_BASE}/stats/runner/{rid}"   # confirmed correct path; no discovery needed


def ff_stats(rid, rt, c, headers):
    global _ff_stats_builder
    if _ff_stats_builder is not None:
        return requests.get(_ff_stats_builder(rid, rt, c), headers=headers, timeout=15)
    last = None
    for i, b in enumerate(_FF_STATS_CANDIDATES):
        u = b(rid, rt, c)
        r = None
        for attempt in range(2):                     # quick retry so a slow response doesn't skip a good path
            try:
                r = requests.get(u, headers=headers, timeout=12)
                break
            except Exception as e:
                log(f"FormFav stats discovery [{i}] {u.replace(FF_BASE,'')} -> timeout/err (try {attempt+1}): {type(e).__name__}")
        if r is None:
            continue
        log(f"FormFav stats discovery [{i}] {u.replace(FF_BASE,'')} -> HTTP {r.status_code}")
        if r.status_code == 200:
            _ff_stats_builder = b
            log(f"FormFav: locked stats endpoint pattern [{i}] = {u.replace(FF_BASE,'')}")
            return r
        last = r
    log("FormFav: NO stats endpoint pattern returned 200 — paste the discovery lines so we can fix the path.")
    return last
...
def enrich_formfav(sb, race_rows, runner_rows, allow_api=True):
    ...
    for rr in order:
        ...
        try:
            time.sleep(FF_PACE_SEC)              # gentle pace to dodge the per-second throttle
            def _search():
                return requests.get(f"{FF_BASE}/stats/runner/search",
            ...
            for attempt in range(2):
                try:
                    ...
                    raise
            # burst throttle? ("Rate exceeded" with NO ratelimit headers) -> wait and retry once
            if s.status_code == 429 and not any("ratelimit" in k.lower() for k in s.headers):
                log(f"FormFav burst-throttled (HTTP 429 'Rate exceeded') — backing off {FF_BACKOFF_SEC}s and retrying.")
                time.sleep(FF_BACKOFF_SEC)
                try: s = _search()
            ...
            if s.status_code in (401, 403):
                log(f"FormFav AUTH FAILED (HTTP {s.status_code}) — the X-API-Key is rejected. "
                    f"The key on Railway is likely wrong/rotated/expired. No more API calls this run.")
                api_blocked = True
                continue
            if s.status_code == 429:
                has_hdr = any("ratelimit" in k.lower() for k in s.headers)
                if has_hdr:
                    ...
                    log("FormFav still burst-throttled after backoff — pausing API for this run, will resume next cycle.")
                api_blocked = True
                continue
            if s.status_code != 200:
                log(f"FormFav search unexpected HTTP {s.status_code} for '{nm}' — skipping.")
            ...
            # transient server error / burst throttle -> back off once and retry
            if st is not None and st.status_code in (429, 500, 502, 503):
                log(f"FormFav stats HTTP {st.status_code} for {rid} — backing off {FF_BACKOFF_SEC}s and retrying once.")
                time.sleep(FF_BACKOFF_SEC)
                try: st = ff_stats(rid, rt, ctry, headers)
                except Exception: pass
            ...
            if st.status_code in (401, 403):
                ...
                api_blocked = True
                continue
            if st.status_code == 429 and any("ratelimit" in k.lower() for k in st.headers):
