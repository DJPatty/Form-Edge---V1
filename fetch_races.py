...
FF_PACE_SEC   = 0.4         # pause between FormFav calls — avoids the per-second "Rate exceeded" throttle
FF_BACKOFF_SEC = 5          # if throttled (429 "Rate exceeded"), wait this long and retry once before giving up
...
_ff_stats_builder = None   # locked once discovered (per process / per run)
    ...
    for rr in order:
        ...
        try:
            ...
            if st.status_code == 429:
                log(f"FormFav rate limit reached (HTTP 429) on stats call — resets in ~{_fmt_secs(_ff_reset_secs(st))}. "
                    "No more API calls this run; still applying cache.")
                api_blocked = True
                continue
            if st.status_code != 200:
                log(f"FormFav stats unexpected HTTP {st.status_code} for runnerId {rid} — skipping.")
                seen[key] = {"id": None, "stats": None}
                continue
            stats = st.json()
            calls += 1
