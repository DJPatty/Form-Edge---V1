...
def enrich_formfav(sb, race_rows, runner_rows):
    if not FORMFAV_KEY:
        ...
        return
    log(f"FormFav: key present (len={len(FORMFAV_KEY)}), starting enrichment via {FF_BASE}")
    ...
    probed = False
    api_blocked = False   # set when quota/auth fails — stop API calls but KEEP applying cache hits
    ...
    for rr in order:
        ...
        try:
            ...
            if not probed:   # one-time diagnostic so the logs show exactly what FormFav returns
                snippet = (s.text or "")[:180].replace("\n", " ")
                log(f"FormFav probe: GET search -> HTTP {s.status_code}; remaining≈{remaining}; body[:180]={snippet}")
                probed = True
            ...
            if s.status_code == 429:
                log("FormFav rate limit reached (HTTP 429) — daily quota exhausted. "
            ...
            if st.status_code == 429:
                log("FormFav rate limit reached (HTTP 429) on stats call — no more API calls this run; still applying cache.")
                api_blocked = True
                continue
...
def main():
    ...
    # FormFav only needs refreshing occasionally — a horse's form doesn't change minute-to-minute.
    # Odds, though, are written every run below. So we gate enrichment by time since it last ran,
    # using a tiny marker row in Supabase. This keeps the 1-min odds cron from burning the daily quota.
    if formfav_due(sb):
        enrich_formfav(sb, race_rows, runner_rows)
        mark_formfav_run(sb)
    else:
        log("FormFav: skipped this run (not due yet — odds still refreshed). Enrichment runs every "
            f"{FF_EVERY_HOURS}h.")
