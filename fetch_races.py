...
CERT_PEM  = os.environ["BETFAIR_CERT"]   # contents of client-2048.crt
KEY_PEM   = os.environ["BETFAIR_KEY"]    # contents of client-2048.key
...
def list_markets(token):
    ...
    r = requests.post(f"{BETTING}/listMarketCatalogue/", json=body,
                      headers=bf_headers(token), timeout=20)
    r.raise_for_status()
    markets = r.json()
